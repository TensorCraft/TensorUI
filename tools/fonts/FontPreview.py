...
class FontViewer(QMainWindow):
    ...
    def load_bfont(self):
        ...
        try:
            with open(filename, 'rb') as f:
                # Read widths (128 characters + 1 height)
                widths = struct.unpack("130B", f.read(130))

                # Last width entry is the common height for all characters
                char_height = widths[-2]
                char_widths = widths[:-2]  # The first 128 entries are character widths
                font_size = widths[-1]
                print("Font Size:", font_size)

                # Read character pixel data for each ASCII character
                char_data = []
            ...
            # Render all characters on the canvas
    ...
    def render_characters(self, char_data, char_widths, char_height):
        ...
        for i in range(32, 127):  # Display printable ASCII characters (32 to 126)
            ...
            # If the character is too wide for the current row, wrap to the next line
            if x + width > canvas.width() - 10:
                x = 10
                y += max_line_height + 10
                max_line_height = 0

            # If the current y position is beyond the canvas, expand the canvas
            ...
            # Draw character pixels on the canvas
            for row in range(char_height):
                for col in range(width):
                    if char_pixels[row, col] == 1:
                        painter.drawPoint(x + col, y + row)

            # Update x position and line height for next character
            x += width + 10
